...
def find_cython_implementation_modules():
    ...
    extensions = []
    for root, _, files in os.walk("elsai_utilities"):
        for file in files:
...
def rename_so_files():
    ...
    for root, dirs, files in os.walk("elsai_utilities"):
        for file in files:
...
setup(
    name="elsai_utilities",
    version=version_module.__version__,
    description="The **Elsai Utilities** package provides helper classes for **chunking** and **converting documents** as part of retrieval-augmented generation (RAG) and vector database ingestion workflows.",
    ...
    # CRITICAL: Proper package_d`ata configuration
    package_data={
        '': ['*.so', '*.pyd', '*.dll'],  # Include all compiled extensions
        'elsai_utilities': ['**/*.so', '**/*.pyd', '**/*.dll'],
    },
)