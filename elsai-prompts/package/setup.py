...
def find_cython_implementation_modules():
    ...
    extensions = []
    for root, _, files in os.walk("elsai_prompts"):
        for file in files:
...
def process_prompt_file(root: str, file: str):
...
setup(
    ...
    description="elsai_prompts - A lightweight Python class to fetch prompts from the Pezzo prompt management API based on prompt name, environment, and project configuration.",
