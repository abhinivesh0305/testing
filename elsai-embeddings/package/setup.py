...
def find_cython_implementation_modules():
    ...
    extensions = []
    for root, _, files in os.walk("elsai_embeddings"):
        for file in files:
...
def process_embedding_file(root: str, file: str):
...
def rename_so_files():
    ...
    for root, _, files in os.walk("elsai_embeddings"):
        for file in files:
            process_embedding_file(root, file)
...
setup(
    name="elsai_embeddings",
    version=version_module.__version__,
    description="**elsai-embeddings** provides an easy interface to embed queries (single text) or documents (list of texts) using Azure OpenAI's embeddings models. It includes logging for embedding operations and error handling.",
