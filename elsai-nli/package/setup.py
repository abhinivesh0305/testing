...
def find_cython_implementation_modules():
    ...
    extensions = []
    for root, _, files in os.walk("elsai_nli"):
        for file in files:
...
def process_nli_file(root: str, file: str):
...
setup(
    ...
    description="The **Elsai NLI (Natural Language Interface)** package enables users to query structured data sources such as CSV files using natural language. It leverages LLM-powered agents to interpret and convert human language into structured operations, allowing intuitive data exploration and analysis.",
